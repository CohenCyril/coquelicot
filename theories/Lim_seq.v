...
Proof.
  case => l Hl ; exists (Rbar_opp l) ; by apply is_lim_seq_opp.
Qed.
Lemma Lim_seq_opp :
  forall u,
...
Proof.
intros u.
transitivity (Rbar_mult (-1) (Lim_seq u)).
rewrite -Lim_seq_scal.
apply Lim_seq_ext.
move => n ; ring.
have H : -1 < 0.
rewrite -Ropp_0 ; apply Ropp_lt_contravar, Rlt_0_1.
case: (Lim_seq u) => [l | | ] //=.
apply Rbar_finite_eq ; ring.
by case: Rle_dec (Rlt_not_le _ _ H).
by case: Rle_dec (Rlt_not_le _ _ H).
Qed.
...
Lemma ex_lim_seq_decr (u : nat -> R) (M : R) :
  (forall n, (u (S n)) <= (u n)) -> (forall n, M <= u n)
    -> ex_lim_seq u /\ is_finite (Lim_seq u).
Proof.
  intros.
...
Proof.
  ...
  move => n ; by apply Ropp_le_contravar.
  move/ex_lim_seq_opp => Hu.
  rewrite Lim_seq_opp => Hu'.
  split.
  move: Hu ; apply ex_lim_seq_ext.
  move => n ; by rewrite Ropp_involutive.
  case: (Lim_seq u) Hu' => //.
Qed.
...
Lemma ex_lim_seq_adj (u v : nat -> R) :
  ...
  -> ex_lim_seq u /\ ex_lim_seq v /\ Lim_seq u = Lim_seq v /\ is_finite (Lim_seq u).
Proof.
  move => Hu Hv H0.
  suff H : forall n, u n <= v n.
  suff Eu : ex_lim_seq u /\ is_finite (Lim_seq u).
