...
Proof.
  move => Ha.
  move: (is_lim_seq_scal _ x l _ Ha (refl_equal _)) => {Ha} Ha.
...
Proof.
  ...
  have : exists N, forall n, (N <= n)%nat -> r ^ (S n) * M / INR (fact (S n)) < eps.
    ...
    rewrite -(Rmult_0_r r) ; apply is_lim_seq_scal with 0 => //.
