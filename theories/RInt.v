...
Lemma Riemann_sum_Chasles_0 :
  forall {V} {VV : NormedVectorSpace V R} (f : R -> V) (M : R) (x : R) ptd (eps : posreal),
...
Proof.
  ...
  + case: (Rle_dec (SF_h ptd) x) => Hx1.
    - replace (minus (plus (Riemann_sum f (SF_cut_down (SF_cons (x0, y1) ptd) x))
      ...
      rewrite /minus 2?(Riemann_sum_cons _ _ _ (x0, y1)) SF_cut_down_h.
...
Definition RInt_val {V} {VV : VectorSpace V R} (f : R -> V) (a b : R) (n : nat) :=
  scal ((b-a)/(INR n+1)) (foldr plus zero (SF_val_ly f a b n)).

Lemma RInt_val_swap :
  forall V (VV : VectorSpace V R) (f : R -> V) (a b : R) (n : nat),
...
Proof.
  ...
  case: Hex => If Hex.
  generalize (proj1 (filterlim_locally _ If) Hex) => {Hex} /= Hex.
  ...
  assert (forall g, pointed_subdiv (ptd g) ->
    ...
    move => g Hg ; replace (minus (Riemann_sum f (ptd g)) If)
      with (minus (scal (sign (b - a)) (Riemann_sum f (ptd g))) If).
  apply Hex.
...
Proof.
have H : (forall a b i, (S i < size (unif_part a b n))%nat ->
  nth 0 (unif_part a b n) (S i) - nth 0 (unif_part a b n) i = (b-a)/(INR n + 1)).
...
case: Rle_dec => Hab.
(* a <= b *)
  rewrite /SF_val_ly ; case: (unif_part a b n) (H a b) => [| h s] /=.
...
(* ~ a <= b *)
  ...
  case: (unif_part b a n) (H b a) => [| h s] /=.
  move => _ ; field ; apply Rgt_not_eq ; intuition.
...
(* * alpha = Rmin (Rmin alpha1 alpha2) alpha3 *)
  ...
  rewrite /Riemann_sum (SF_Chasles _ lx1 (phi (SF_h ptd))) /=.
  replace (_-_) with
    ((ly0 * (lx1 - lx0) - 1* RInt_seq (SF_cut_down' (SF_map phi ptd) lx1 (phi (SF_h ptd))) Rplus Rmult 0)
    + (Int_SF ly (RList.cons lx1 lx) - 1* RInt_seq (SF_cut_up' (SF_map phi ptd) lx1 (phi (SF_h ptd))) Rplus Rmult 0))
    ; [ | by ring_simplify].
...
(* partie [lx0;lx1] *)
  ...
  (SF_map phi (SF_cut_down' ptd lx1 (SF_h ptd)) : @SF_seq R).
...
(* partie [lx1 ; b] *)
  ...
  (SF_map phi (SF_cut_up' ptd lx1 (SF_h ptd)) : @SF_seq R).
...
(* preuve de H *)
  ...
  change (RInt_seq (SF_map phi ptd) Rplus Rmult 0) with (Riemann_sum phi ptd).
  rewrite Hx1 pointed_pty2.
  change zero with R0.
...
Lemma ex_RInt_Reals_aux_2 (f : R -> R) (a b : R) :
    ...
    pointed_subdiv ptd1 -> pointed_subdiv ptd2 ->
      Rabs
        (RInt_seq (SF_seq_f2 (fun x y => f (g1 x y) - f(g2 x y))
          (unif_part a b n) 0) Rplus Rmult 0) < eps.
  move => g1 g2 ptd1 pdt2 H1 H2.
  replace (RInt_seq _ _ _ _ : R) with
    (RInt_seq (SF_map f (SF_seq_f2 g1 (unif_part a b n) 0)) Rplus Rmult 0 -
    RInt_seq (SF_map f (SF_seq_f2 g2 (unif_part a b n) 0)) Rplus Rmult 0).
  apply HIf.
  exact: H1.
  exact: H2.
  rewrite !SF_map_f2.
  elim: (unif_part a b n) {1 3 5}(0) => /= [ x0 | h0].
  rewrite /RInt_seq /= ; ring.
  case => /= [ | h1 t] IH x0.
  rewrite /RInt_seq /= ; ring.
...
(* * faire rentrer Rabs dans l'intégrale *)
  ...
  replace (RInt_seq
    (SF_seq_f2 (fun x y : R => Rabs (f (g1 x y) - f (g2 x y)))
    (unif_part a b n) 0) Rplus Rmult 0) with
    (RInt_seq (SF_seq_f2 (fun x y : R => (f (h1 x y) - f (h2 x y)))
    (unif_part a b n) 0) Rplus Rmult 0).
  rewrite -(Rabs_pos_eq (RInt_seq
    (SF_seq_f2 (fun x y : R => f (h1 x y) - f (h2 x y)) (unif_part a b n) 0)
    Rplus Rmult 0)).
  apply HIf.
  ...
  apply IH.
  by apply Hsort.
  elim: (unif_part a b n) {1 3}(0) => [x0 | x0].
  by rewrite /RInt_seq /= .
  case => [ | x1 t] IH z.
  by rewrite /RInt_seq /= .
...
(* * dernière étape :-) *)
  ...
  suff Hlub : is_lub (F 0 (unif_part a b n)) (RInt_seq (SF_seq_f2
...
Proof.
  case => If Hex.
  exists If ; by apply ex_RInt_Reals_aux_2.
Qed.
...
Proof.
  ...
  set s' := {|
      ...
      SF_t := [seq (u * fst X + v, u * snd X + v)
                 | X <- SF_t s] |}.
  rewrite Riemann_sum_cons (Riemann_sum_cons _ _ _ (u * x0 + v,u * y0 + v) s') /=.
  rewrite IH.
  apply f_equal2 => //=.
  rewrite scal_assoc /=.
  apply f_equal2 => //= ; ring.
...
Proof.
  ...
  apply filterlim_locally => eps.
  generalize (proj1 (filterlim_locally _ _) H1 (pos_div_2 (pos_div_2 eps))) => {H1} H1.
  generalize (proj1 (filterlim_locally _ _) H2 (pos_div_2 (pos_div_2 eps))) => {H2} H2.
  case: H1 => d1 H1.
  case: H2 => d2 H2.
...
intros [Hab Hbc] H1 H2.
apply filterlim_locally => eps.
generalize (proj1 (filterlim_locally _ _) H1 (pos_div_2 eps)) ; case => {H1} d1 /= H1.
generalize (proj1 (filterlim_locally _ _) H2 (pos_div_2 eps)) ; case => {H2} d2 /= H2.
...
replace (minus (minus (Riemann_sum f y1) (Riemann_sum f y2)) (minus l1 l2))
  ...
  rewrite (Riemann_sum_cons _ _ _ (x0,y0) {| SF_h := SF_h y; SF_t := SF_t y ++ SF_t y2 |}) /=.
  rewrite Riemann_sum_cons /=.
  rewrite /minus -plus_assoc.
  apply f_equal.
  by apply IH.
...
Proof.
  ...
  - move => H2.
    ...
    rewrite H /minus plus_zero_l in H1 => {H}.
    generalize (proj1 (filterlim_locally _ _) H2 (pos_div_2 eps)) ; case => /= {H2} d2 H2.
  ...
  - apply filterlim_locally => /= eps.
    ...
    assert ( H := scal_zero_l (K := R) (Riemann_sum f (SF_nil a))).
    rewrite H /minus plus_zero_l in H2 => {H}.
    generalize (proj1 (filterlim_locally _ _) H1 (pos_div_2 eps)) ; case => /= {H1} d1 H1.
  ...
  - rewrite -Hac' in H1 Hbc' H2 Hac |- * => {c Hac'}.
    ...
    rewrite H /minus plus_zero_l opp_plus => {H y Hstep Hy}.
    generalize (proj1 (filterlim_locally _ _) H1 (pos_div_2 eps)) ; case => /= {H1} d1 H1.
    generalize (proj1 (filterlim_locally _ _) H2 (pos_div_2 eps)) ; case => /= {H2} d2 H2.
    assert (exists y, seq_step (SF_lx y) < Rmin d1 d2 /\
      pointed_subdiv y /\
    ...
    rewrite (double_var eps).
    rewrite -norm_opp /minus opp_plus opp_opp in H2.
    apply Rle_lt_trans with (2 := Rplus_lt_compat _ _ _ _ H1 H2).
    apply Rle_trans with (2 := norm_triangle _ _).
    apply Req_le, f_equal.
