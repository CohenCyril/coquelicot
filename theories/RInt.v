...
Lemma ex_RInt_correct_aux_2 (f : R -> R) (a b : R) :
  forall If, is_RInt f a b If
      -> is_lim_seq (RInt_val f a b) If.
Proof.
...
Lemma SF_sup_fun_finite f a b n m M :
  ...
  -> forall t, Rmin a b <= t <= Rmax a b -> is_finite (SF_sup_fun f a b n t).
Admitted.
...
Proof.
  wlog: a b /(a <= b) => [Hw | Hab ] Hex.
    ...
    apply RiemannInt_P1 ;
    by apply Hw.
  have : {If : R | forall eps : posreal, exists alpha : posreal,
    forall (ptd : SF_seq) (H : pointed_subdiv ptd) 
      (Hstep : seq_step (SF_lx ptd) < alpha) 
      (Ha : SF_h ptd = Rmin a b) (Hb : last (SF_h ptd) (SF_lx ptd) = Rmax a b),
      Rabs (If - signe (b-a) * Riemann_sum f ptd) < eps}.
  ...
  have Hn : forall eps : posreal, {n : nat | (b-a)/2^n < alpha eps}.
    move => eps.
    case: (Rle_lt_dec (alpha eps) (b-a)) => H.
    case: (nfloor_ex ((ln (b-a) - ln (alpha eps))/ln 2)) => [ | n [ _ Hn]].
    apply Rdiv_le_pos.
    apply -> Rminus_le_0.
    apply ln_le.
    by apply Ealpha.
    ...
    rewrite ln_mult.
    apply Rminus_gt.
    apply Rgt_minus in Hn.
    by replace (ln (alpha eps) + ln (2 ^ S n) - ln (b - a))
      with (ln (2 ^ S n) - (ln (b - a) - ln (alpha eps))) by ring.
    by apply Ealpha.
    by intuition.
    exact: Rlt_R0_R2.
    apply Rlt_trans with (/2).
    by apply Rinv_0_lt_compat, Rlt_R0_R2.
    exact: ln_lt_2.
    exists O ; rewrite pow_O /Rdiv Rinv_1 Rmult_1_r ; exact: H.
...
(* Construire phi *)
  ...
  exists phi.
(* Construire psi *)
  set psi := SF_sup_r (fun t => Rabs (f t - phi t)) a b n.
  exists psi.

have H : forall t, a <= t <= b ->
  is_finite (SF_sup_fun (fun t0 : R => Rabs (f t0 - phi t0)) a b n t).
  move => t H ;
  case: (ex_RInt_ub f a b Hex) => M HM.
  case: (ex_RInt_lb f a b Hex) => m Hm.
  apply (SF_sup_fun_finite _ a b _ 0 (Rabs (M-m))) => [x Hx_ab | ].
  split.
  apply Rabs_pos.
  apply Rle_trans with (2 := Rle_abs _).
  apply Rabs_le_between ; rewrite Ropp_minus_distr'.
  split ; apply Rplus_le_compat, Ropp_le_contravar.
  by apply Hm.
  rewrite /phi /sf_SF_val_fun ; case: Rle_dec => //= _.
  move: Hx_ab ; rewrite /Rmin /Rmax ; case: Rle_dec => // _ Hx_ab.
  rewrite SF_val_fun_rw.
  case: RInt_part_nat => [ [i [Hx Hi]] | [Hx Hi] ] ; simpl projT1.
  apply HM ; rewrite /Rmin /Rmax ; case: Rle_dec => // _.
  pattern b at 3 ; replace b with (a + 2^S n * (b - a) / 2 ^ S n).
  pattern a at 1 ; replace a with (a + 0 * (b - a) / 2 ^ S n).
  apply Rminus_le_0 in Hab.
  split ; apply Rplus_le_compat_l ; apply Rmult_le_compat_r, Rmult_le_compat_r ;
  try by intuition.
  ...
  exact: Rle_0_1.
  apply Rlt_le, Rinv_0_lt_compat ; by intuition.
  rewrite size_mkseq in Hi ; apply lt_S_n, lt_INR in Hi.
  simpl ; rewrite -pow2_INR ; rewrite S_INR in Hi.
  apply Rle_trans with (2*(INR i + 1)).
  ...
  try by intuition.
  apply Rlt_le, Rinv_0_lt_compat ; by intuition.
  apply Rle_trans with (1 := Rle_0_1).
  apply Rminus_le_0 ; ring_simplify ; simpl.
  replace (2*2 ^ n - 2) with (2*(2^n - 1)) by ring.
  apply Rmult_le_pos.
  by apply Rlt_le, Rlt_R0_R2.
  rewrite -Rminus_le_0 ; apply pow_R1_Rle ; rewrite -{1}(Rplus_0_l 1) ; by intuition.
  apply Rlt_le, Rinv_0_lt_compat ; by intuition.
  apply Rminus_le_0 ; ring_simplify ; apply Rle_0_1.
  field ; apply Rgt_not_eq ; by intuition.
  field ; apply Rgt_not_eq ; by intuition.
...
(* Partie 1 *)
  move => t Ht_ab.
  apply Rbar_finite_le ; simpl.
  rewrite H.
  apply (SF_sup_ge_f (fun t => Rabs (f t - phi t))), Ht_ab.
  move: Ht_ab ; rewrite /Rmin /Rmax ; case: Rle_dec => //.
...
(* * SF_lx ptd = RInt_part a b n *)
  ...
  simpl => _ ; rewrite /seq_step /= ; by apply Ealpha.
  case: t IH => /= [ | h1 t] IH Hnth.
  rewrite /seq_step /= ; by apply Ealpha.
  rewrite /seq_step /= ; apply (Rmax_case_strong (Rabs (h1 - h0))) => /= _.
  ...
  by apply HIf.
  rewrite /signe /=.
  move: (proj1 (Rminus_le_0 _ _) Hab) ; case: Rle_dec => //= H0 _.
  ...
  replace (head 0 (RInt_part a b n) :: behead (RInt_part a b n))
    with (RInt_part a b n) by auto.
  rewrite -nth_last size_mkseq nth_mkseq //= pow2_INR -H0 ; 
  field ; apply Rgt_not_eq ; by intuition.
  rewrite /SF_sorted SF_lx_f2 ;
  replace (head 0 (RInt_part a b n) :: behead (RInt_part a b n))
    with (RInt_part a b n) by auto ;
...
(* * faire rentrer Rabs dans l'intégrale *)
  ...
  apply HIf.
    ...
    apply (IH x0 (ptd_cons _ _ H1) (ptd_cons _ _ H2)).
    apply lt_S_n, Hi.
  elim: (RInt_part a b n) (RInt_part_sort a b n Hab) {2}(0) => [Hsort x0 | x0].
...
(* * phi (g x y) = f (h x y) *)
  ...
  have H2 : pointed_subdiv ptd2.
    ...
    rewrite -Rdiv_1 ; apply Rdiv_le_pos.
    rewrite Rplus_comm -Rminus_le_0 ; exact: Hab.
    ...
    apply Rmult_lt_0_compat ; by intuition.
    apply Rgt_not_eq ; by intuition.
    apply INR_le ; rewrite pow2_INR /= ; apply pow_R1_Rle.
    rewrite -{1}(Rplus_0_l 1) ; apply Rplus_le_compat_r, Rle_0_1.
    apply SSR_leq ; by intuition.
    apply SSR_leq ; by intuition.
    apply le_antisym ; apply not_lt.
    have Hij : nth 0 (RInt_part a b n) i < nth 0 (RInt_part a b n) (pow2 n).
      apply Rle_lt_trans with 
        (g1 (nth 0 (RInt_part a b n) i) (nth 0 (RInt_part a b n) (S i))).
      by apply H1.
      case: (proj2 Ht) => {Ht} Ht.
    ...
    by intuition.
    rewrite size_mkseq ; by intuition.
    have Hij : nth 0 (RInt_part a b n) (pow2 n - 1) < nth 0 (RInt_part a b n) (S i).
      apply Rle_lt_trans with 
        (g1 (nth 0 (RInt_part a b n) i) (nth 0 (RInt_part a b n) (S i))).
      by apply Ht.
      case: (proj2 H1) => {H1} H1.
    ...
    have H0 : forall t, a <= t <= b -> SF_val_fun f a b n t = f (h t).
      ...
      apply (f_equal (fun i => f (a + (2 * INR i + 1) * (b - a) / 2 ^ S n))).
      apply le_antisym ; apply not_lt.
      move: (Rle_lt_trans _ _ _ (proj1 Ht) (proj2 Ht')) => Hij ;
      contradict Hij.
      apply Rle_not_lt, sorted_incr.
      exact: RInt_part_sort.
      by [].
      by intuition.
      move: (Rle_lt_trans _ _ _ (proj1 Ht') (proj2 Ht)) => Hij ;
      contradict Hij.
  ...
  have : forall i, (i < size (RInt_part a b n))%nat ->
    a <= nth 0 (RInt_part a b n) i <= b.
    move => i ; rewrite size_mkseq => Hi ; rewrite nth_mkseq.
    pattern b at 3 ; replace b with (a + 2^n * (b - a) / 2 ^ n) by 
      (field ; apply Rgt_not_eq ; intuition).
    pattern a at 1 ; replace a with (a + 0 * (b - a) / 2 ^ n) by 
      (field ; apply Rgt_not_eq ; intuition).
    apply Rminus_le_0 in Hab.
    ...
    apply Rlt_le ; by intuition.
    rewrite -pow2_INR ; apply le_INR ; by intuition.
    by apply SSR_leq.
    revert ptd1 H1 ;
...
(* * dernière étape :-) *)
  ...
  Focus 2.
  rewrite /RiemannInt_SF SF_sup_subdiv SF_sup_subdiv_val ; 
  case: Rle_dec (Req_le _ _ Hab) => // _ _.
  rewrite (@StepFun_P8 psi _ _ a b _ Hab).
  apply Rlt_le, eps.
  apply (proj1 (ad_SF_sup_r (fun t : R => Rabs (f t - phi t)) a b n) (Req_le _ _ Hab)).
  
  Focus 2.
  rewrite /RiemannInt_SF SF_sup_subdiv SF_sup_subdiv_val ; 
  case: Rle_dec => // _.
  elim: (RInt_part a b n) (RInt_part_sort a b n Hab) => [ Hsort /= | x0].
  by apply Rge_refl.
  case => [ | x1 s] IH Hsort /=.
  by apply Rge_refl.
  apply Rle_ge, Rplus_le_le_0_compat.
  apply Rmult_le_pos.
  rewrite /Sup_fct /Lub_Rbar_ne ; case: ex_lub_Rbar_ne ; 
  case => [l | | ] [ub lub] /=.
  apply Rle_trans with (Rabs (f x0 - phi x0)).
  apply Rabs_pos.
  ...
  exact: Rle_refl.
  exact: Rle_refl.
  rewrite -Rminus_le_0 ; apply Hsort.
  by apply Rge_le, IH, Hsort.
