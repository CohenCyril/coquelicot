...
(* ** c'est la bonne borne sup *)
  ...
  by [].
  by apply eps.
  case => [ | x2 s] IH x0 eps Hle Hlt Hfin.
  split ; rewrite /RInt_seq /=.
  move => val [g [Hnth [Hptd Hval]]].
  rewrite -(proj1 Hval) ; by apply Rle_refl.
  move => ub Hub ; apply: Hub.
  ...
  rewrite RInt_seq_cons /=.
  set F0 := fun (eps : posreal) x => 
    exists t, x1 < t < x2 /\ x = Rabs (f t - phi t) * (x2 - x1) /\ x < eps.
  set set_plus := fun (F : R -> Prop) (G : R -> Prop) (x : R) =>
    exists y, exists z, x = y + z /\ F y /\ G z.
  suff H0 : forall (x : R) eps, F x0 [:: x1, x2 & s] eps x 
    <-> (set_plus (F0 (pos_div_2 eps)) (F x1 (x2::s) (pos_div_2 eps))) x.
  suff H1 : is_lub (set_plus (F0 (pos_div_2 eps)) (F x1 (x2::s) (pos_div_2 eps)))
  ...
  suff H_F0 : is_lub (F0 (pos_div_2 eps)) (real (Sup_fct 
  ...
  move: H_F0 (IH x1 (pos_div_2 eps) (proj2 Hle) (proj2 Hlt) Hfin0).
  ...
  by [].
    ...
    elim: j x2 s Hj {IH Hle Hlt Hfin Hnth Hpdt Hval F0 Ht g0 Hx2' x0 Hx1' i Hi}
  ...
  move: (refl_equal x2) ; case: Req_EM_T => // _ _.
  case: s Hlt {Hpdt IH Hle Hfin F0 Ht Hnth Hval}
    => [ | x3 s] Hlt /=.
  apply refl_equal.
